...
const UpdatePet = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/api/pets/${id}`).then((response) => {
      ...
      setSkill1(response.data.pet.skills[0]);
      setSkill2(response.data.pet.skills[1]);
      setSkill3(response.data.pet.skills[2]);
    });
  }, []);
  ...
  const onSubmitHandler = (e) => {
    ...
    axios
      .put(`http://localhost:8000/api/pets/${id}/edit`, {
        petName,
        petType,
        petDescription,
  ...
  return (
    <div>
      ...
      <h4 className="text-start">Edit: {petName}</h4>
